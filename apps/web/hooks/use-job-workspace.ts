...
import { jobsApi, type GeneratedFile, type JobStatusResponse } from "@/lib/api-client";
...
export function useJobWorkspace() {
  ...
  const refresh = useCallback(async (id: string) => {
    ...
    if (next.status === "COMPLETED") {
      setFiles((await jobsApi.files(id)).files);
    }
    return next;
  }, []);
  ...
  const progress = useMemo(() => job?.progress.progress_percent ?? 0, [job]);
  return {
    run,
    progress,
    job,
